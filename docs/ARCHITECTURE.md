...
Пощупать опыт можно на двух уровнях (подробная установка с нуля — [`INSTALL.md`](INSTALL.md)):

- **Тир 1 — локально (агент + gbrain MCP).** Клон репо + кодинг-агент (Claude Code / Cursor):
  агент читает `hermes/SOUL.md` и `protocols/`, ходит через `knowledge/RESOLVER.md`, а gbrain
  подключается как MCP-сервер (`gbrain serve`) и даёт семантический поиск по теории. Без сервера,
  без Telegram, без крона — методология в чате агента. Минимальный порог входа.
- **Тир 2 — self-host (Hermes gateway + Telegram).** Внешний **hermes-agent** держит гейтвей,
  Telegram-бота, сессии и крон-ритуалы; **gbrain** индексирует `knowledge/`. Полный опыт автора:
  голос, ритуалы, живущий сам по себе агент. Выше порог входа (установка гейтвея, бот, ключи).
