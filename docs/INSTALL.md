...
Первый ход после установки — наполнить базу. Попроси агента **«хочу рассказать о себе»**
(интервью [life-interview](../protocols/life-interview.md): по блокам, по 2–3 вопроса, наполняет
`self/` и `people/`) **и/или** положи насыщенное сырьё в `data/raw/{therapy,journals,chats}/` и
скажи **«переработай инбокс»**. Пока карта пуста, разборы будут generic. Все четыре пути и порядок —
...
Первый ход после установки — онбординг. Напиши боту **«хочу рассказать о себе»** (запустится
[life-interview](../protocols/life-interview.md) и наполнит `self/` + `people/`) **и/или** положи
насыщенное сырьё в `data/raw/{therapy,journals,chats}/` и скажи **«переработай инбокс»** — агент
дистиллирует его в базу. Все четыре пути и порядок — [`ONBOARDING.md`](ONBOARDING.md).
